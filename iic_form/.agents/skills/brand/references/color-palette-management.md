...
| ------------ | ------- | -------------- | ---------------- | ----------- |
| Primary Blue | #2563EB | rgb(37,99,235) | hsl(221,83%,53%) | CTAs, links |
```
...
1. Use one coverage denominator: total analyzed pixel or area coverage, including brand colors, neutrals, and off-brand colors.
2. Allocate in-palette coverage as primary 55-65%, secondary 20-30%, accent 5-10%, and neutral 5-15%; valid allocations must sum to 100% of in-palette coverage.
3. Exclude off-brand colors from that palette distribution, and separately require off-brand coverage to be no more than 20% of total analyzed coverage.
...
The compliance value is deterministic: `round(100 * (1 - offBrandCoverage / totalCoverage))`, clamped to 0-100. `offBrandCoverage` and `totalCoverage` use the same analyzed pixel-area denominator.
...
- Do not use more than 2-3 colors in a single component.
- Do not mix warm and cool tones without intent.
- Do not use pure black (#000) for text; use #111 or a similar alternative instead.
- Do not rely solely on color for meaning; use icons or text too.
