...
Record one result for every approved color combination and interaction state:

| Color Pair                   | Use Case  | State       | Measured Contrast | WCAG Threshold         | Outcome   |
| ---------------------------- | --------- | ----------- | ----------------- | ---------------------- | --------- |
| {foreground} on {background} | Body text | Default     | {ratio}:1         | 4.5:1 (AA normal text) | Pass/Fail |
| {foreground} on {background} | CTA       | Hover/focus | {ratio}:1         | 3:1 (AA UI component)  | Pass/Fail |
...
Scripts can extract the following stable schema:
...
typography:
	heading: { family: string, weight: number, sizes: { desktop: string, mobile: string } }
	body: { family: string, weight: number, sizes: { desktop: string, mobile: string } }
voice:
	traits: [{ name: string, description: string }]
	prohibited: [{ term: string, reason: string }]
logo:
	variants: [{ name: string, description: string }]
	minSize: { digitalWidthPx: number, printWidthMm: number }
```

Human-readable sections must mirror these keys: document one or more primary colors, separate digital and print logo minimum sizes, and structured voice entries with trait descriptions and prohibited-term reasons.
