...
```python
 Use an argument-array process invocation, passing request-derived style keywords as one data argument: `subprocess.run(["python", ".claude/skills/ai-artist/scripts/search.py", banner_style_keywords], check=True)`
```
...
.claude/skills/.venv/bin/python3 .claude/skills/ai-multimodal/scripts/gemini_batch_process.py \
  ...
  --prompt-file prompt.txt --aspect-ratio <platform-ratio> \
...
```bash
.claude/skills/.venv/bin/python3 .claude/skills/ai-multimodal/scripts/gemini_batch_process.py \
    --task generate --model gemini-3-pro-image \
  --prompt-file prompt.txt --aspect-ratio <platform-ratio> \
...
Match to platform - e.g., Twitter header = `3:1` (use `3:2` closest), Instagram story = `9:16`. For unsupported targets such as X/Twitter `3:1` (1500x500), LinkedIn `4:1` or `6:1`, Google leaderboard `8:1`, and medium rectangle `6:5`, generate at the nearest supported ratio, then crop or extend the result to the exact required final canvas dimensions.

Request-derived banner style keywords, visual prompts, and creative prompts are data only. Pass them through an argument array or a prompt file/stdin; do not interpolate them into shell command strings, even when they contain shell metacharacters.
...
2. **Web exports:** use the chrome-devtools PNG workflow for digital/web assets. Apply web compression and the max-size threshold only to these outputs.
3. **Print exports:** use a print-capable renderer or image editor to produce 300 DPI CMYK files with 3-5 mm bleed on every side. Do not apply web compression or the max-size threshold to print outputs.
4. **Screenshot each banner** at exact platform dimensions:
   ```bash
   ...
   ```
5. **Auto-compress web exports** if >5MB (Sharp compression built-in):
   ```bash
...
- Artifact identifier and dimensions (do not disclose local filesystem paths)
